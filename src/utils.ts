...
export async function resolveThemeColorStyles(
	...
	return Object.fromEntries(
		await Promise.all(resolvedThemes),
	) as ThemesWithColorStyles
}
...
abstract class PostsCollationGroup implements CollationGroup<'blog'> {
	title: string
	url: string
	collations: Collation<'blog'>[]

	constructor(title: string, url: string, collations: Collation<'blog'>[]) {
		this.title = title
		this.url = url
		this.collations = collations
	}

	sortCollationsAlpha(): Collation<'blog'>[] {
		this.collations.sort((a, b) => a.title.localeCompare(b.title))
		return this.collations
	}

	sortCollationsLargest(): Collation<'blog'>[] {
		this.collations.sort((a, b) => b.entries.length - a.entries.length)
		return this.collations
	}

	sortCollationsMostRecent(): Collation<'blog'>[] {
	...
	add(item: CollectionEntry<'blog'>, collationTitle: string): void {
	...
	match(rawKey: string): Collation<'blog'> | undefined {
		return this.collations.find((entry) => entry.title === rawKey)
	}

	matchMany(rawKeys: string[]): Collation<'blog'>[] {
...
export class SeriesGroup extends PostsCollationGroup {
	...
	private constructor(title: string, url: string, items: Collation<'blog'>[]) {
		super(title, url, items)
	}
	// Factory method to create a SeriesGroup instance with async data fetching
	static async build(blog?: CollectionEntry<'blog'>[]): Promise<SeriesGroup> {
		const sortedPosts = blog || (await getSortedPosts())
